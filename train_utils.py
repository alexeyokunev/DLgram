...
def register_dataset(proj_dir):
    path_to_train_coco = osp.join(proj_dir, TRAIN_COCO_NAME)
...
def modify_cfg(cfg, ckpt_path, dataset_name, path_to_train_coco, num_epochs):
    ...
    cfg.data.samples_per_gpu = len(train_coco['images'])
    cfg.gpu_ids = range(1)
    ...
    max_per_img = 2000
    max_rpn = 16000
    cfg.test_cfg.rpn.nms_pre = max_rpn
    cfg.test_cfg.rpn.nms_post = max_rpn
    cfg.test_cfg.rpn.max_num = max_rpn
    cfg.test_cfg.rcnn.max_per_img = max_per_img

    nms_thr = 0.3 #0.15
    iou_thr = 0.4 #0.2
    cfg.test_cfg.rpn.nms_thr = nms_thr
    cfg.test_cfg.rcnn.nms.iou_thr = iou_thr
    
    return cfg

def train_loop(path_to_cfg, path_to_ckpt, proj_dir, num_epochs=400):
    clses = register_dataset(proj_dir)    
          
    # read and modify config
    cfg = Config.fromfile(path_to_cfg)
    path_to_train_coco = osp.join(proj_dir, TRAIN_COCO_NAME)
    cfg = modify_cfg(cfg, path_to_ckpt, DATASET_NAME, path_to_train_coco, num_epochs)
    path_to_dst_cfg = osp.join(proj_dir, CFG_NAME)
...
if __name__ == '__main__':
    train_loop(path_to_cfg, path_to_ckpt, proj_dir, num_epochs=400)